...
const Like = ({ isAuthenticated, authUser, postId, likes, createLike, removeLike }) => {

   const likesLengthCount = (likesLength) => {
      if (likesLength <= 0) {
         return '';
      } else return likesLength;
   };

   const onLikeClick = (e) => {
      e.preventDefault();
      const existedLike = likes.find(like => like.user === authUser.id);
      if (isAuthenticated) {
         if (existedLike) {
   ...
   return (
      <div className={s.like} onClick={onLikeClick}>
         <button>
         ...
         <span>{likesLengthCount(likes.length)}</span>
      </div>
   );
};
